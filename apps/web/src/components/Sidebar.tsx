...
export function Sidebar() {
  ...
  const isActive = (status?: string, fav?: boolean, path?: string) => {
      ...
      <aside className="w-64 bg-slate-900 h-screen flex flex-col border-r border-slate-800 fixed left-0 top-0 z-40">
        ...
        <div className="px-6 mb-8">
          {user ? (
            <Link
              href="/profile"
              className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl border border-slate-800 group relative hover:bg-slate-800 transition-colors"
            >
              <div className="w-10 h-10 rounded-full bg-slate-700 overflow-hidden">
                <img
                  src={
                    user.avatarUrl ||
                    `https://api.dicebear.com/9.x/notionists/svg?seed=${user.username}&backgroundColor=b6e3f4,c0aede,d1d4f9`
                  }
                  alt={user.username}
            ...
            <Link
              href="/login"
              className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl border border-slate-800 hover:bg-slate-800 transition-colors"
            >
          ...
          <div className="space-y-1">
            ...
            >
             ...
             <Link
                href="/taste-analysis"
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-colors ${
                  isActive(undefined, undefined, 'taste-analysis')
                    ? 'bg-purple-500/10 text-purple-400'
                    : 'text-slate-400 hover:text-white hover:bg-slate-800/50'
                }`}
              >
                <Sparkles className="w-5 h-5" />
                <span>{t('tasteAnalysis')}</span>
            </Link>
          </div>
